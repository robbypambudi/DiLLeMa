...
from app.pipeline.tasks import run_pipeline_with_error_handling
...
def create(
    ...
    background_tasks.add_task(
        run_pipeline_with_error_handling, pipeline_service, response
    )
    return BaseResponse(message="File created successfully", data=response)


@router.post(
    "/{file_id}/retry", tags=["post"], response_model=BaseResponse[ResponseFiles]
)
@inject
def retry(
    file_id: uuid.UUID,
    ...
    return BaseResponse(message="File queued for retry", data=response)
...
def get_file(
    ...
    return BaseResponse(message="File retrieved successfully", data=file)
