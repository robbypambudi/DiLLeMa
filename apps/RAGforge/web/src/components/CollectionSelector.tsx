...
export function CollectionSelector({ appState, updateState }: CollectionSelectorProps) {
  const { collections, selectedCollection, isLoading } = appState
  ...
  const fetchCollections = async () => {
    ...
    try {
      ...
      if (data.status === 'success' && data.data) {
        updateState({ collections: data.data.map((col: { id: string; collection_name: string; description?: string }) => ({
          id: col.id,
  ...
  const handleCollectionChange = (collection: Collection) => {
    if (isLoading || selectedCollection?.id === collection.id) return
    updateState({
    ...
    <aside className="flex shrink-0 flex-col border-b bg-surface p-4 md:w-72 md:border-b-0 md:border-r md:p-5 lg:w-80" aria-label="Document collections">
      ...
      {loading ? (
        ...
        <div role="alert" className="rounded-xl border border-destructive/30 bg-destructive/5 p-4 text-sm">
          ...
          <Button variant="outline" size="sm" className="mt-3" onClick={fetchCollections}>Try again</Button>
        </div>
        ...
        <>
          <div className="md:hidden">
            ...
            <select id="mobile-collection" className="field-control" value={selectedCollection?.id || ''} disabled={isLoading} onChange={(event) => {
          ...
          <div className="hidden min-h-0 space-y-2 overflow-y-auto md:block">
            {collections.map((collection) => {
              ...
              return (
                <button key={collection.id} type="button" aria-pressed={selected} disabled={isLoading} onClick={() => handleCollectionChange(collection)}
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block break-words text-sm font-semibold">{collection.name}</span>
